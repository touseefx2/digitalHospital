...
render(){  
  const {city,speciality,specialityItems,cityItem,aid} = this.state;
 ...
 if(this.props.mydoctorusers)
  {  
      this.item = this.props.mydoctorusers.map( (doctoraccount) =>{ 
         ...
         {
          if(doctoraccount.address.city == cityy &&  doctoraccount.speciality == specialityy && doctoraccount.uid != auth().createUser.uid)
          { 
            x=true;
            return(
              <ShowDoctorsCardS aid={aid} navigate={this.props.navigation} d={doctoraccount}/>
            )
          }
          }
          else if(doctoraccount.uid != auth().currentUser.uid){
            return(
            <ShowDoctorsCardS aid={aid} navigate={this.props.navigation} d={doctoraccount}/>
          )
        }       
...
<View  style={{width:"100%",height:0.3,backgroundColor:"#307ecc"}} />
  {cityy !="" && specialityy !="" && (
   ...
   {cityy =="" && specialityy =="" && (
    <Text  style={{fontSize:14,margin:5,color:"grey"}}>All Doctor List</Text>
  )}
       <ScrollView >
       <View style={{marginTop:"1%",margin:5}}>
        {cityy =="" && specialityy =="" && this.item}
