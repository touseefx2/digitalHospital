  ...
  this.unsubscribe = firestore().collection("appointments").orderBy("createdat", "desc").onSnapshot((docs)=>{
    ...
    if(docs.size>0)
    {
      ...
      docs.forEach((data) => {
        ...
        this.unsubscribee =  firestore().collection("users").doc(d.doctorid).onSnapshot((u)=>{
         if(u.exists)
         {
          dbupdate.update({
            doctorinfo:{
        ...
        if(d.active =="Process")
        {
          ...
          if(CurrentDate == "Equal")
          {
            ...
            const obj = {aid:aid,refdid:d.refdid,address:d.draddress,cancelreason:d.cancelreason,cancelby:d.cancelby,email:d.dremail,speciality:d.speciality,active:d.active,exist:d.exist,doctorinfo:d.doctorinfo,date:d.date,showdate:d.showdate,time:d.time,duration:d.duration,fees:d.fees,doc:d.document,docname:d.documentname,patientid:d.patientid,doctorid:d.doctorid}
             app.push(obj);
...
checkdocmntfileexist()
{
  ...
  componentDidMount(){
    ...
    this.timerInterval = setInterval(() => {this.dynamicheckAppntmnt()},10000)
    this.checkdocmntfileexist()
}
